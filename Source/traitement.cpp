...
void Traitement::AfficherContenuImage(const Mat Image_In){
    ...
    cvtColor(getImage(),l_grayMat, COLOR_BGR2GRAY);

    //Affichage de l'image originale
    namedWindow("originale", WINDOW_AUTOSIZE);
	imshow("originale", l_grayMat);

    namedWindow("Image apres Traitement", WINDOW_NORMAL);
    imshow("Image apres Traitement", Image_In);
    //resizeWindow("dffd", 620, 620);  
    waitKey(0);
    destroyWindow("Image apres Traitement");
}
...
Mat Traitement::Seuillage_simple(){
    ...
    cvtColor(l_Image_In,l_grayMat, COLOR_BGR2GRAY);
...
Mat Traitement::Gaussien(){
    ...
    cvtColor(l_Image_In,grayMat, COLOR_BGR2GRAY);
...
void Traitement::Sauvegarder(){
    ...
    if(l_reponse1== "Y"){
        ...
        l_exist = filesystem::exists(l_path) ;
